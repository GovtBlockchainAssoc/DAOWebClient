...
export default {
  ...
  computed: {
    ...
    rewardSalary () {
      const data = this.assignment.proposal.assets.find(o => o.key === 'weekly_reward_salary')
      return (data && data.value && parseFloat(data.value).toFixed(2)) || 0
    },
    voteSalary () {
      const data = this.assignment.proposal.assets.find(o => o.key === 'weekly_vote_salary')
      return (data && data.value && parseFloat(data.value).toFixed(2)) || 0
    },
    startPhase () {
