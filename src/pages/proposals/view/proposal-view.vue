...
export default {
  ...
  computed: {
    ...
    votepow () {
      const obj = this.proposal.assets.find(o => o.key === 'votepow_amount')
      return obj && obj.value
    },
    hypha () {
    ...
    .col-xs-12.col-md-8.offset-md-2
      ...
      .row
        ...
        q-card.col-xs-12.col-md-4.q-mt-lg.full-height
          ...
          q-card-section.text-center(v-if="hypha || seeds || votepow")
            strong Payout Amounts
          q-card-section(v-if="hypha || seeds || votepow")
            p(v-if="hypha") {{ hypha }}
            p(v-if="seeds") {{ seeds }}
            p(v-if="votepow") {{ votepow }}
