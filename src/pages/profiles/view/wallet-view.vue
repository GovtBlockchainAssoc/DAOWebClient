...
.q-pa-lg
  .row
    .tokens-wallet-mobile(v-if="!$q.platform.is.desktop")
      ...
      .token-info.row.flex.items-center
        img.icon(src="~assets/icons/votepow.svg")
        div
          .name VOTEPOW
          .amount {{ new Intl.NumberFormat().format(tokens.votepow, { style: 'currency' }) }}
      .token-info.row.flex.items-center
    ...
    .tokens-wallet(v-if="$q.platform.is.desktop")
      ...
      transition(
        ...
        .token-info.row.flex.items-center(v-if="show3" style="transition-delay: 0.2s")
          ...
          div
            .name VOTEPOW
            .amount {{ new Intl.NumberFormat().format(parseInt(tokens.votepow), { style: 'currency' }) }}
      transition(
        appear
        enter-active-class="animated slideInRight"
        leave-active-class="animated slideOutRight"
